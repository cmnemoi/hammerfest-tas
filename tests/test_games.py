...
from tests.fakes import DELUXE_OPTIONS
...
def test_presets_belong_to_their_game(world, caplog):
    ...
    exit_code = world.run("speedrun-no-rng", "-g", OTHER)
...
def test_unknown_game_is_rejected_listing_the_games(world, caplog):
    exit_code = world.run("-g", "hammerfest")
...
def test_recordings_of_each_game_are_kept_apart(world):
    with_second_game(world, served=HAMMERFEST)
    world.run_named("run", "sandbox", "-g", HAMMERFEST)
    world.eternaldev_serves(OTHER)
    world.run_named("run", "-g", OTHER)
    world.eternaldev_goes_down()

    world.run_named("run", "-g", HAMMERFEST)
...
def test_status_tells_eternaldev_runs_and_which_games_it_serves(world, capsys):
    ...
    assert f"{OTHER}: not built (`hftas build`), not served" in out
    assert f"{HAMMERFEST}: built, served" in out


def test_status_tells_eternaldev_is_down_and_still_lists_the_games(world, capsys):
    world.eternaldev_goes_down()
    ...
    out = capsys.readouterr().out
    assert exit_code == 1
    assert "eternaldev: not running" in out
    assert f"{HAMMERFEST}: built\n" in out


def test_status_lists_the_options_presets_and_recordings_of_a_game(world, capsys):
    world.run("pap-rng")

    world.hftas("status")

    out = capsys.readouterr().out
    assert "options (deluxe): " + ", ".join(DELUXE_OPTIONS) in out
    assert "(default)" in out and "pap-rng              boost, insight" in out
    assert "recordings: pap-rng" in out
