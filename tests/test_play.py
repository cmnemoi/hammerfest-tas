...
def test_options_can_be_added_to_and_removed_from_a_preset(world):
    world.play("sandbox", "-o", "ninja", "-x", "insight")
...
def test_mistyped_option_is_rejected_before_the_game_starts(world, caplog):
    ...
    exit_code = world.play("-o", "tornade")
...
def test_unknown_profile_is_rejected_instead_of_playing_as_a_new_account(world, caplog):
    ...
    exit_code = world.play("sandbox", "--profile", "ful")
...
@pytest.mark.parametrize("command", [["play", "sandbox"], ["record", "sandbox"]])
def test_games_start_under_libtas_by_default(world, command):
    world.hftas(*command)
...
def test_ruffle_only_starts_the_game_without_libtas(world):
    world.play("sandbox", "--ruffle-only")

    assert not world.loader.launches[-1].tas


def test_options_lists_the_options_of_the_presets_mode(world, capsys):
    world.hftas("options")

    assert "boost" in capsys.readouterr().out.split()


def test_the_mode_can_be_changed(world):
    world.play("--mode", "solo", "-o", "mirror")
